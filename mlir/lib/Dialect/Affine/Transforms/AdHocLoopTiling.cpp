...
struct AdHocLoopTiling : public affine::impl::AffineAdHocLoopTilingBase<AdHocLoopTiling> {
  AdHocLoopTiling() = default;
  explicit AdHocLoopTiling(uint64_t cacheSizeBytes, bool avoidMaxMinBounds = true)
...
mlir::affine::createAdHocLoopTilingPass(uint64_t cacheSizeBytes) {
  return std::make_unique<AdHocLoopTiling>(cacheSizeBytes);
}
std::unique_ptr<OperationPass<func::FuncOp>>
...
void AdHocLoopTiling::runOnOperation() {
  ...
  LLVM_DEBUG(llvm::dbgs() << "[" DEBUG_TYPE "] " << AdHocLoopTiling::kDefaultTileSize <<"yodelayheeehooooo\n");
...
constexpr unsigned AdHocLoopTiling::kDefaultTileSize;
