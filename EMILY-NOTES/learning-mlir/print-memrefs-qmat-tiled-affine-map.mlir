...
// messing around with affine maps
  "func.func"() <{function_type = (memref<16x16xi8>, memref<16x16xi8, strided<[1, 16]>>, memref<16x16xi32, strided<[16,1]>>) -> (), sym_name = "simple_matmul_tiled_affine_map"}> ({
  ^bb0(%arg0: memref<16x16xi8>, %arg1: memref<16x16xi8, strided<[1,16]>>, %arg2: memref<16x16xi32, strided<[16,1]>>):
    ...
    %alloc = memref.alloc() {alignment = 1 : i64} : memref<16x16xi32>
    %alloc_strided = memref.reinterpret_cast %alloc to offset: [0], sizes:[16,16], strides:[1,16] : memref<16x16xi32> to memref<16x16xi32, strided<[1, 16]>>
    linalg.fill ins(%three : i32) outs(%alloc_strided : memref<16x16xi32, strided<[1, 16]>>)

    %00 = memref.alloc() : memref<8x16x4xi32, affine_map<(d0, d1, d2) -> (d0 * 64 + d1 * 4 + d2)>>
    // linalg.fill ins(%three : i32) outs(%00 : memref<8x16x4xi32, affine_map<(d0, d1, d2) -> (d0 * 64 + d1 * 4 + d2)>>)
    ...
    // map as symbols 's1', 's2' and 's3'.
     %size0 = arith.constant 2 : index
     %size1 = arith.constant 2 : index
     %size2 = arith.constant 2 : index
    ...
    // %1 = memref.subview %00[%i, %j, %k][%size0, %size1, %size2][%x, %y, %z]
    //   : memref<8x16x4xi32, affine_map<(d0, d1, d2) -> (d0 * 64 + d1 * 4 + d2)>> to
    //     memref<?x?x?xi32,
    //       affine_map<(d0, d1, d2)[s0, s1, s2, s3] -> (d0 * s1 + d1 * s2 + d2 * s3 + s0)>>

    // %alloc2 = memref.alloc() {alignment = 1 : i64} : memref<16x16xi32, affine_map<(d0, d1)-> (d0*16+d1)>>
    // linalg.fill ins(%three : i32) outs(%alloc2 : memref<16x16xi32, affine_map<(d0, d1)-> (d0*16+d1)>>)
    // %x = arith.constant 16 : index
    // %y = arith.constant 1 : index
    // %leftTile2 = memref.subview %alloc2[%i,%j][2,16][%x,%y] : memref<16x16xi32, affine_map<(d0, d1)-> (d0*16+d1*1)>> to memref<2x16xi32, affine_map<(d0, d1)[s0, s1]-> (d0*s0+d1*s1)>>

   // %alloc3 = memref.alloc(){alignment = 1 :i64} :  memref<2x16xi32, affine_map<(d0, d1)[s0, s1]-> (d0*s0+d1*s1)>>
  

    } // end of i for
    } // end of j for
    } // end of k for
   // %cast3 = memref.cast %00 :  memref<8x16x4xi32, affine_map<(d0, d1, d2) -> (d0 * 64 + d1 * 4 + d2)>> to memref<*xi32>
   // func.call @printMemrefI32(%cast3) : (memref<*xi32>) -> ()
   // 

    //print the matrix allocated
    %cast2 = memref.cast %alloc_strided : memref<16x16xi32, strided<[1, 16]>> to memref<*xi32>
   // func.call @printMemrefI32(%cast2) : (memref<*xi32>) -> ()
    memref.dealloc %alloc : memref<16x16xi32>
    memref.dealloc %00 : memref<8x16x4xi32, affine_map<(d0, d1, d2) -> (d0 * 64 + d1 * 4 + d2)>>
    "func.return"() : () -> ()
  }) : () -> ()
...
func.func @main() {
  ...
  //call matmul
  call @simple_matmul_tiled_affine_map(%0, %alloc_strided, %alloc_000) : (memref<16x16xi8>, memref<16x16xi8, strided<[1, 16]>>, memref<16x16xi32,strided<[16,1]>>) -> ()
