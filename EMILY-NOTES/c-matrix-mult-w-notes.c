...
"blocks" are set to make step-by-step examples easier

command to run example:
rm test.o out/regular_output out/tiled_general_output;clear; gcc c-matrix-mult-w-notes.c -o test.o;./test.o; diff out/regular_output out/tiled_general_output
...
void multmatTiledGeneral(squareMat *a, squareMat *b, squareMat *c,
  ...
  for (i = 0; i < n; i++)
    for (j = 0; j < n; j++)
      c->mat[i][j] = 0.0;
printf("kk\tjj\ti\tj\tk\t| C[i][j] += A[i][k] * B[k][j]\n");
      ...
      for (i = 0; i < n; i++) {
        for (j = jj; j < jj + bsize; j++) {
          ...
          for (k = kk; k < kk + bsize; k++) {
            ...
            // printf("kk\tjj\ti\tj\tk\t| C[i][j] += A[i][k] * B[k][j]\n");
            printf("%d\t%d\t%d\t%d\t%d\t| C[%d][%d] += A[%d][%d] * B[%d][%d]\n",kk,jj,i,j,k,i,j,i,k,k,j);
          }
          c->mat[i][j] = sum;
        }
      }
